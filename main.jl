using Plots, LaTeXStrings, Distributions, LinearAlgebra, Random, Crayons, Revise
...
function run_benchmarks()
    ...
    if !model_approx(bl2019_carma, carma)
        error(
            "BL transformation does not match original model!\n\n$(bl2019_carma)\n\n$(carma)",
        )
    end
    ...
    for _ in 1:num_tests
        ...
        if !model_approx(carma, bl2019_double_conj)
            error(
                "BL: Found model that doesn't return to itself after conjugation.\n\n$(carma)\n\n$(bl2019_conj_arma)\n\n$(bl2019_double_conj)",
            )
        end
        ...
        if !model_approx(conj_arma, bl2019_conj_arma)
            error(
                "BL and our method provide different conjugates:\n\nBL: $(bl2019_conj_arma)\n\n $(conj_arma)",
            )
        end

        if !model_approx(carma, double_conj)
            error(
                "Found model that doesn't return to itself after conjugation.\n\n$(carma)\n\n$(conj_arma)\n\n$(double_conj)",
            )
        end
    end
    println("Matched BL2019 on random tests.")

    return log_info("</> Finished benchmarks.\n$(SEPERATOR)")
end
...
function stationary_process_tracking()
    ...
    initial_m = [3.0; -1.0]
    initial_P = [2.0 0.5; 0.5 1.0]
    println("Initial mean:\n$(initial_m)\n")
    println("Initial cov:\n$(initial_P)\n")

    initial_m_star, initial_P_star = conjugate(
        arma, sample_time, initial_m, initial_P, true
    )
    ...
    arma_upper_conf, arma_lower_conf = arma_means +
                                       GAUSSIAN_CONFIDENCE_95 * sqrt.(arma_covs),
    arma_means - GAUSSIAN_CONFIDENCE_95 * sqrt.(arma_covs)

    upsampling = 10
    carma_steps = 0:(sample_time / upsampling):(sample_time * num_steps)
    carma_means, carma_covs = step_model(
        carma,
        initial_m_star,
        initial_P_star,
        num_steps * upsampling,
        sample_time / upsampling,
    )
    carma_upper_conf, carma_lower_conf = carma_means +
                                         GAUSSIAN_CONFIDENCE_95 * sqrt.(carma_covs),
    carma_means - GAUSSIAN_CONFIDENCE_95 * sqrt.(carma_covs)

    p = plot(;
        grid=false,
        xlabel="Forward prediction (s)",
        ylabel="Process estimate",
        ylim=[-30, 30],
    ...
    return log_info("</> Finished stationary process prediction example.\n$(SEPERATOR)")
end
...
function transform_integrated_OU()
    ...
    discrete_zeros = Vector{Float64}()
    discrete_vars = Vector{Float64}()
    last = -1
    ...
    plot!(
        ...
        alpha_range,
        discrete_zeros;
        title="Non-trivial ARMA(2, 1) parameters",
        label=L"z_d",
        legend=:bottomright,
        legendfontsize=LEGEND_FONT_SIZE,
        ylabel="Zero position",
        color=BLUE,
        linewidth=LINE_WIDTH,
        ylim=[-1.0, 0.0],
        grid=false,
    ...
    return log_info("</> Finished integrated OU example.\n$(SEPERATOR)")
end
...
function transform_singer()
    ...
    discrete_zeros::Matrix{ComplexF64} = zeros((2, length(alpha_range)))
    discrete_vars = Vector{Float64}()
    println("Ranging alpha values on: ", alpha_range)
    ...
    plot!(
        twinx(p),
        alpha_range,
        real.(discrete_zeros[1, :]);
        title="Non-trivial ARMA(3,2) parameters",
        label=L"z_d",
        legend=:bottomright,
        legendfontsize=LEGEND_FONT_SIZE,
        ylabel="Zero position",
    ...
    return log_info("</> Finished Singer example.\n$(SEPERATOR)")
end
